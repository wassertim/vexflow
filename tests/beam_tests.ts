...
import { Beam, Note, Stem, Voice } from '../src';
import { IFactoryParams } from '../src/types/factory';
import { IFormatOptions } from '../src/types/formatter';
...
export const BeamTests = (function() {
  ...
  return {
    ...
    multi: function(options) {
      var vf = Test.makeFactory(options);
      var stave = vf.Stave();
    ...
    breakSecondaryBeams: function(options) {
      var vf = Test.makeFactory(options, 600, 200);
      var stave = vf.Stave(<IFactoryParams>{ y: 20 });
      var score = vf.EasyScore();
    ...
    slopey: function(options) {
      var vf = Test.makeFactory(options, 350, 140);
      var stave = vf.Stave(<IFactoryParams>{ y: 20 });
      var score = vf.EasyScore();
    ...
    autoStem: function(options) {
      ...
      var beams = [
        vf.Beam(<IFactoryParams>{ notes: notes.slice(0, 2), options: { autoStem: true } }),
        vf.Beam(<IFactoryParams>{ notes: notes.slice(2, 4), options: { autoStem: true } }),
        vf.Beam(<IFactoryParams>{ notes: notes.slice(4, 6), options: { autoStem: true } }),
        vf.Beam(<IFactoryParams>{ notes: notes.slice(6, 8), options: { autoStem: true } }),
        vf.Beam(<IFactoryParams>{ notes: notes.slice(8, 10), options: { autoStem: true } }),
        vf.Beam(<IFactoryParams>{ notes: notes.slice(10, 12), options: { autoStem: true } }),
      ];
    ...
    mixed: function(options) {
      ...
      [[0, 4], [4, 8], [8, 12]].forEach(function(range) {
        vf.Beam(<IFactoryParams>{ notes: voice1.getTickables().slice(range[0], range[1]) });
      });

      [[0, 4], [4, 8], [8, 12]].forEach(function(range) {
        vf.Beam(<IFactoryParams>{ notes: voice2.getTickables().slice(range[0], range[1]) });
      });
    ...
    mixed2: function(options) {
      var vf = Test.makeFactory(options, 450, 180);
      var stave = vf.Stave(<IFactoryParams>{ y: 20 });
      var score = vf.EasyScore();
      ...
      vf.Beam(<IFactoryParams>{ notes: voice.getTickables().slice(0, 12) });
      vf.Beam(<IFactoryParams>{ notes: voice2.getTickables().slice(0, 12) });
    ...
    dotted: function(options) {
      ...
      var notes = voice.getTickables();
      vf.Beam(<IFactoryParams>{ notes: notes.slice(0, 4) });
      vf.Beam(<IFactoryParams>{ notes: notes.slice(4, 8) });
      vf.Beam(<IFactoryParams>{ notes: notes.slice(8, 12) });
    ...
    partial: function(options) {
      ...
      vf.Beam(<IFactoryParams>{ notes: notes.slice(3, 9) });
      vf.Beam(<IFactoryParams>{ notes: notes.slice(9, 13) });
      vf.Beam(<IFactoryParams>{ notes: notes.slice(13, 17) });
    ...
    insane: function(options) {
      ...
      var notes = voice.getTickables();
      vf.Beam(<IFactoryParams>{ notes: notes.slice(0, 4) });
      vf.Beam(<IFactoryParams>{ notes: notes.slice(4, 7) });
    ...
    outlier: function(options) {
      ...
      vf.Formatter()
        .joinVoices([voice])
        .formatToStave([voice], stave, <IFormatOptions>{ stave: stave });
    ...
    tabBeamsUp: function(options) {
      var vf = Test.makeFactory(options, 600, 200);
      var stave = vf.TabStave(<IFactoryParams>{ y: 20 });
      ...
      var notes = specs.map(function(noteSpec: IStaveNoteStruct) {
        var tabNote = vf.TabNote(noteSpec);
        tabNote.render_options.draw_stem = true;
        return tabNote;
      });

      vf.Beam(<IFactoryParams>{ notes: <Note[]>notes.slice(1, 7) });
      vf.Beam(<IFactoryParams>{ notes: <Note[]>notes.slice(8, 11) });
      vf.Tuplet(<IFactoryParams>{ notes: <Note[]>notes.slice(8, 11), options: { ratioed: true } });

      var voice = vf.Voice().setMode(Voice.Mode.SOFT).addTickables(notes);
    ...
    tabBeamsDown: function(options) {
      var vf = Test.makeFactory(options, 600, 250);
      var stave = vf.TabStave(<IFactoryParams>{ options: { num_lines: 10 } });
      ...
      vf.Tuplet(<IFactoryParams>{ notes: <Note[]>notes.slice(8, 11), options: { location: -1 } });
      vf.Tuplet(<IFactoryParams>{ notes: <Note[]>notes.slice(11, 14), options: { location: -1 } });
    ...
    autoTabBeams: function(options) {
      ...
      var voice = vf.Voice().setMode(Voice.Mode.SOFT).addTickables(notes);
      var beams = Beam.applyAndGetBeams(voice, -1);
    ...
    tabBeamsAutoStem: function(options) {
      var vf = Test.makeFactory(options, 600, 300);
      var stave = vf.TabStave();
      ...
      vf.Beam(<IFactoryParams>{ notes: <Note[]>notes.slice(0, 8), options: { autoStem: true } });
      // Stems should format up
      vf.Beam(<IFactoryParams>{ notes: <Note[]>notes.slice(8, 12), options: { autoStem: true } });
    ...
    complexWithAnnotation: function(options) {
      var vf = Test.makeFactory(options, 500, 200);
      var stave = vf.Stave(<IFactoryParams>{ y: 40 });
      ...
      const notes = noteStructs.map(function(note: IStaveNoteStruct) {
        return vf.StaveNote(note).addModifier(0, vf.Annotation(<IFactoryParams>{ text: '1', vJustify: 'above' }));
      });

      const notes2 = notesStructs2.map(function(note: IStaveNoteStruct) {
        return vf.StaveNote(note).addModifier(0, vf.Annotation(<IFactoryParams>{ text: '3', vJustify: 'below' }));
      });

      vf.Beam(<IFactoryParams>{ notes: <Note[]>notes });
      vf.Beam(<IFactoryParams>{ notes: <Note[]>notes2 });
    ...
    complexWithArticulation: function(options) {
      ...
      const notes = noteStructs.map(function(note: IStaveNoteStruct) {
        return vf.StaveNote(note).addModifier(0, vf.Articulation(<IFactoryParams>{ type: 'am', position: 'above' }));
      });

      const notes2 = noteStructs2.map(function(note: IStaveNoteStruct) {
        return vf.StaveNote(note).addModifier(0, vf.Articulation(<IFactoryParams>{ type: 'a>', position: 'below' }));
      });
