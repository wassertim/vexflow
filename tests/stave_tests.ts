...
import { IStaveOptions, IStaveTextOptions } from '../src/types/stave';
...
export const StaveTests = (function() {
  return {
    Start: function() {
    ...
    draw: function(options, contextBuilder) {
      const ctx = new contextBuilder(options.elementId, 400, 150);
    ...
    drawOpenStave: function(options, contextBuilder) {
      ...
      let stave = new Stave(10, 10, 300, <IStaveOptions>{ left_bar: false });
      stave.setContext(ctx);
      stave.draw();

      stave = new Stave(10, 150, 300, <IStaveOptions>{ right_bar: false });
      stave.setContext(ctx);
      stave.draw();
    ...
    drawMultipleMeasures: function(options, contextBuilder) {
      ...
      const notesBar1 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['c/4'], duration: 'q' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['d/4'], duration: 'q' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['b/4'], duration: 'qr' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'g/4'], duration: 'q' }),
      ];
      ...
      const notesBar2_part1 = [
        ...
        new StaveNote(<IStaveNoteStruct>{ keys: ['g/4'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['e/4'], duration: '8' }),
      ];
    ...
    drawRepeats: function(options, contextBuilder) {
      ...
      // bar 2 - juxtaposing second bar next to first bar
      const staveBar2 = new Stave(staveBar1.width + staveBar1.x,
        staveBar1.y, 250);
      ...
      const staveBar3 = new Stave(staveBar2.width + staveBar2.x,
        staveBar2.y, 50);
      staveBar3.setContext(ctx).draw();
      const notesBar3 = [new StaveNote(<IStaveNoteStruct>{ keys: ['d/5'], duration: 'wr' })];

      // Helper function to justify and draw a 4/4 voice
      Formatter.FormatAndDraw(ctx, staveBar3, notesBar3);

      // bar 4 - juxtaposing third bar next to third bar
      const staveBar4 = new Stave(staveBar3.width + staveBar3.x,
    ...
    drawVoltaTest: function(options, contextBuilder) {
      ...
      const notesmm1 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['c/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm2 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['d/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm3 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['e/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm4 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['f/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm5 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['g/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm6 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['a/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm7 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['b/4'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm8 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['c/5'], duration: 'w' }),
      ];
      // Helper function to justify and draw a 4/4 voice
      ...
      const notesmm9 = [
        new StaveNote(<IStaveNoteStruct>{ keys: ['d/5'], duration: 'w' }),
      ];
    ...
    drawTempo: function(options, contextBuilder) {
      ...
      function drawTempoStaveBar(width, tempo, tempo_y, notes?) {
        ...
        const notesBar = notes || [
          new StaveNote(<IStaveNoteStruct>{ keys: ['c/4'], duration: 'q' }),
          new StaveNote(<IStaveNoteStruct>{ keys: ['d/4'], duration: 'q' }),
          new StaveNote(<IStaveNoteStruct>{ keys: ['b/4'], duration: 'q' }),
          new StaveNote(<IStaveNoteStruct>{ keys: ['c/4'], duration: 'q' }),
        ];
      ...
      drawTempoStaveBar(250, { name: 'Andante', note: '8', bpm: 120 }, -20, [
        ...
        new StaveNote(<IStaveNoteStruct>{ keys: ['d/4'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['g/4'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['e/5'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['c/4'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['d/4'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['g/4'], duration: '8' }),
        new StaveNote(<IStaveNoteStruct>{ keys: ['e/4'], duration: '8' }),
    ...
    drawStaveText: function(options, contextBuilder) {
      const ctx = new contextBuilder(options.elementId, 900, 140);
    ...
    drawStaveTextMultiLine: function(options, contextBuilder) {
      const ctx = new contextBuilder(options.elementId, 900, 200);
      const stave = new Stave(300, 40, 300);
      stave.setText('Violin', Modifier.Position.LEFT, <IStaveTextOptions>{ shift_y: -10 });
      stave.setText('2nd line', Modifier.Position.LEFT, <IStaveTextOptions>{ shift_y: 10 });
      stave.setText('Right Text', Modifier.Position.RIGHT, <IStaveTextOptions>{ shift_y: -10 });
      stave.setText('2nd line', Modifier.Position.RIGHT, <IStaveTextOptions>{ shift_y: 10 });
      stave.setText('Above Text', Modifier.Position.ABOVE, <IStaveTextOptions>{ shift_y: -10 });
      stave.setText('2nd line', Modifier.Position.ABOVE, <IStaveTextOptions>{ shift_y: 10 });
      stave.setText('Left Below Text', Modifier.Position.BELOW,
        <IStaveTextOptions>{ shift_y: -10, justification: TextNote.Justification.LEFT });
      stave.setText('Right Below Text', Modifier.Position.BELOW,
        <IStaveTextOptions>{ shift_y: 10, justification: TextNote.Justification.RIGHT });
      stave.setContext(ctx).draw();
    ...
    factoryAPI: function(options) {
      const vf = Test.makeFactory(options, 900, 200);
      const stave = vf.Stave(<IFactoryParams>{ x: 300, y: 40, width: 300 });
      stave.setText('Violin', Modifier.Position.LEFT, <IStaveTextOptions>{ shift_y: -10 });
      stave.setText('2nd line', Modifier.Position.LEFT, <IStaveTextOptions>{ shift_y: 10 });
      vf.draw();
