...
import { Annotation, Barline, Beam, Formatter, Stave, StaveNote } from '../src';
...
export const RhythmTests = (function() {
  return {
    Start: function() {
      const runTests = Test.runTests;
      QUnit.module('Rhythm');
      runTests('Rhythm Draw - slash notes', this.drawBasic.bind(this));
      runTests('Rhythm Draw - beamed slash notes', this.drawBeamedSlashNotes.bind(this));
      runTests('Rhythm Draw - beamed slash notes, some rests', this.drawSlashAndBeamAndRests.bind(this));
      runTests('Rhythm Draw - 16th note rhythm with scratches', this.drawSixtenthWithScratches.bind(this));
      runTests('Rhythm Draw - 32nd note rhythm with scratches', this.drawThirtySecondWithScratches.bind(this));
    },

    drawSlash: function(options, contextBuilder) {
      const ctx = new contextBuilder(options.elementId, 350, 180);
    ...
    drawBasic: function(options, contextBuilder) {
      ...
      staveBar1.setBegBarType(Barline.type.DOUBLE);
      staveBar1.setEndBarType(Barline.type.SINGLE);
      ...
      const notesBar1 = [
        new StaveNote(
          <IStaveNoteStruct>{ keys: ['b/4'], duration: '1s', stem_direction: -1 }),
      ];
      ...
      const staveBar2 = new Stave(staveBar1.width + staveBar1.x,
        staveBar1.y, 120);
      staveBar2.setBegBarType(Barline.type.SINGLE);
      staveBar2.setEndBarType(Barline.type.SINGLE);
      staveBar2.setContext(ctx).draw();

      // bar 2
      const notesBar2 = [
        new StaveNote(
          <IStaveNoteStruct>{ keys: ['b/4'], duration: '2s', stem_direction: -1 }),
        new StaveNote(
          <IStaveNoteStruct>{ keys: ['b/4'], duration: '2s', stem_direction: -1 }),
      ];
    ...
    drawSlashAndBeamAndRests: function(options,
      ...
      const notesBar1_part1 = [
        ...
        new StaveNote(<IStaveNoteStruct>{ keys: ['b/4'], duration: '8s', stem_direction: -1 }),
      ];

      notesBar1_part1[0].addModifier(0, (new Annotation('C7')).setFont(
        'Times', Test.Font.size + 2));
      ...
      // Helper function to justify and draw a 4/4 voice
      Formatter.FormatAndDraw(ctx, staveBar1,
      ...
      notesBar2[0].addModifier(0, (new Annotation('F')).setFont('Times',
        Test.Font.size + 2));
      // Helper function to justify and draw a 4/4 voice
      Formatter.FormatAndDraw(ctx, staveBar2, notesBar2);
