...
export const ThreeVoicesTests = (function() {
  ...
  function createThreeVoicesTest(noteGroup1, noteGroup2, noteGroup3, setup) {
    return function(options) {
      ...
      const voices: Voice[] = noteGroups.map(score.voice.bind(score));
  ...
  return {
    Start: function() {
      ...
      run('Auto Adjust Rest Positions - Two Voices', this.autoRestTwoVoices);
      run('Auto Adjust Rest Positions - Three Voices #1', this.autorestthreevoices);
      run('Auto Adjust Rest Positions - Three Voices #2', this.autorestthreevoices2);
    },
    ...
    autoRestTwoVoices: function(options) {
      ...
      function createMeasure(measureTitle, width, alignRests) {
        const stave = vf.Stave(<IFactoryParams>{ x: x, y: 50, width: width }).setBegBarType(1);
        x += width;
        ...
        const voices: Voice[] = [
          ...
          [vf.TextNote(<IStaveNoteStruct>{ text: measureTitle, line: -1, duration: '1', smooth: true })],
        ].map(score.voice.bind(score));

        beams = beams
          .concat(Beam.applyAndGetBeams(voices[0], 1))
          .concat(Beam.applyAndGetBeams(voices[1], -1));

        vf.Formatter()
          .joinVoices(voices)
          .formatToStave(voices, stave, <IFormatOptions>{ align_rests: alignRests });
      }
    ...
    autorestthreevoices: function(options) {
      const vf = Test.makeFactory(options, 850, 200);
      const score = vf.EasyScore();
      let x = 10;
      ...
      function createMeasure(measureTitle, width, alignRests) {
        ...
        const voices = [
          ...
          score.voice([
            vf.TextNote(<IStaveNoteStruct>{ text: measureTitle, duration: '1', line: -1, smooth: true }),
            vf.TextNote(<IStaveNoteStruct>{ text: '', duration: '1', line: -1, smooth: true }),
          ], { time: '8/4' }),
        ];
    ...
    autorestthreevoices2: function(options) {
      ...
      function createMeasure(measureTitle, width, alignRests) {
        ...
        const voices = [
          ...
          score.voice([
            vf.TextNote(<IStaveNoteStruct>{ text: measureTitle, duration: 'h', line: -1, smooth: true }),
          ], { time: '2/4' }),
        ];
