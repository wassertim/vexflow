...
import { Barline, Beam, Bend, Note, Stave, StaveNote, Stroke, Voice } from '../src';
...
export const StrokesTests = (function() {
  return {
    Start: function() {
    ...
    brushRollRasquedo: function(options) {
      ...
      // bar 1
      const stave1 = vf.Stave(<IFactoryParams>{ width: 250 }).setEndBarType(Barline.type.DOUBLE);
      ...
      notes2[0]
        .addStroke(0, new Stroke(3));
      notes2[1]
        .addStroke(0, new Stroke(4));
      notes2[2]
        .addStroke(0, new Stroke(5));
      notes2[3]
        .addStroke(0, new Stroke(6))
        .addAccidental(0, vf.Accidental(<IFactoryParams>{ type: 'bb' }))
        .addAccidental(1, vf.Accidental(<IFactoryParams>{ type: 'bb' }))
        .addAccidental(2, vf.Accidental(<IFactoryParams>{ type: 'bb' }));
    ...
    arpeggioDirectionless: function(options) {
      ...
      // bar 1
      const stave1 = vf.Stave(<IFactoryParams>{ x: 100, width: 500 }).setEndBarType(Barline.type.DOUBLE);
      ...
      const graceNoteGroup = vf.GraceNoteGroup(<IFactoryParams>{ notes: graceNotes, slur: false });
      ...
      notes1[2]
        .addStroke(0, new Stroke(7))
        .addAccidental(1, vf.Accidental(<IFactoryParams>{ type: 'b' }))
        .addModifier(0, graceNoteGroup);
      notes1[3]
        .addStroke(0, new Stroke(7))
        .addModifier(0, vf.NoteSubGroup(<IFactoryParams>{ notes: [
          <Note>vf.ClefNote(<IFactoryParams>{ type: 'treble', options: { size: 'default', annotation: '8va' } }),
        ] }));
    ...
    multiNotationAndTab: function(options) {
      const vf = Test.makeFactory(options, 400, 275);
      const score = vf.EasyScore();
      ...
      vf.TabStave(<IFactoryParams>{ y: 100 })
        .addClef('tab')
      ...
      const tabNotes2 = [
        vf.TabNote(<IStaveNoteStruct>{ positions: [{ str: 6, fret: 3 }], duration: '4' }),
        vf.TabNote(<IStaveNoteStruct>{ positions: [{ str: 6, fret: 3 }], duration: '4' }),
        vf.TabNote(<IStaveNoteStruct>{ positions: [{ str: 6, fret: 3 }], duration: '4' }),
        vf.TabNote(<IStaveNoteStruct>{ positions: [{ str: 6, fret: 3 }], duration: '4' }),
      ];

      const voices: Voice[] = [notes1, notes2, tabNotes1, tabNotes2].map(score.voice.bind(score));
    ...
    drawTabStrokes: function(options) {
      const vf = Test.makeFactory(options, 600, 200);
      const stave1 = vf.TabStave(<IFactoryParams>{ width: 250 }).setEndBarType(Barline.type.DOUBLE);
      ...
      // bar 2
      const stave2 = vf.TabStave(<IFactoryParams>{ x: stave1.width + stave1.x, width: 300 })
        .setEndBarType(Barline.type.DOUBLE);
    ...
    notesWithTab: function(options) {
      const vf = Test.makeFactory(options, 500, 300);

      const stave = vf.Stave(<IFactoryParams>{ x: 15, y: 40, width: 450 }).addClef('treble');

      const notes = [
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['b/4', 'd/5', 'g/5'], stem_direction: -1, duration: '4' })
          .addAccidental(1, vf.Accidental(<IFactoryParams>{ type: 'b' }))
          .addAccidental(0, vf.Accidental(<IFactoryParams>{ type: 'b' })),
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/5', 'd/5'], stem_direction: -1, duration: '4' }),
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['b/3', 'e/4', 'a/4', 'd/5'], stem_direction: 1, duration: '8' }),
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['a/3', 'e/4', 'a/4', 'c/5', 'e/5', 'a/5'], stem_direction: 1, duration: '8' })
          .addAccidental(3, vf.Accidental(<IFactoryParams>{ type: '#' })),
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['b/3', 'e/4', 'a/4', 'd/5'], stem_direction: 1, duration: '8' }),
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['a/3', 'e/4', 'a/4', 'c/5', 'f/5', 'a/5'], stem_direction: 1, duration: '8' })
          .addAccidental(3, vf.Accidental(<IFactoryParams>{ type: '#' }))
          .addAccidental(4, vf.Accidental(<IFactoryParams>{ type: '#' })),
      ];

      const tabstave = vf.TabStave(<IFactoryParams>{ x: stave.x, y: 140, width: 450 })
        .addClef('tab')
        .setNoteStartX(stave.getNoteStartX());
