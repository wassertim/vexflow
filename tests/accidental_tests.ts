...
import Vex, { Accidental, Formatter, ModifierContext, Note, StaveNote, Stem, TickContext, TimeSigNote, Voice } from '../src';

import type { IFactoryParams } from '../src/types/factory';
import type { IStaveNoteStruct } from '../src/types/note';
...
export const AccidentalTests = (function() {
  ...
  return {
    Start: function() {
      ...
      Test.runTests('Basic', this.basic);
      Test.runTests('Stem Down', this.basicStemDown);
      Test.runTests('Cautionary Accidental', this.cautionary);
      Test.runTests('Accidental Arrangement Special Cases', this.specialCases);
      Test.runTests('Multi Voice', this.multiVoice.bind(this));
      Test.runTests('Microtonal', this.microtonal);
      Test.runTests('Microtonal (Iranian)', this.microtonal_iranian);
      Test.runTests('Sagittal', this.sagittal);
      test('Automatic Accidentals - Simple Tests', this.autoAccidentalWorking);
      Test.runTests('Automatic Accidentals', this.automaticAccidentals0);
      Test.runTests('Automatic Accidentals - C major scale in Ab', this.automaticAccidentals1);
      Test.runTests('Automatic Accidentals - No Accidentals Necsesary', this.automaticAccidentals2);
      Test.runTests('Automatic Accidentals - Multi Voice Inline', this.automaticAccidentalsMultiVoiceInline);
      Test.runTests('Automatic Accidentals - Multi Voice Offset', this.automaticAccidentalsMultiVoiceOffset);
      Test.runTests('Factory API', this.factoryAPI);
    },

    basic: function(options) {
      const vf = Test.makeFactory(options, 700, 240);
      const newAccid = makeNewAccid(vf);
      vf.Stave({ x: 10, y: 10, width: 550 } as IFactoryParams);

      const notes = [
        vf.StaveNote({ keys: ['c/4', 'e/4', 'a/4'], duration: '1' } as IStaveNoteStruct)
          .addAccidental(0, newAccid('b'))
          .addAccidental(1, newAccid('#')),

        vf.StaveNote({ keys: ['d/4', 'e/4', 'f/4', 'a/4', 'c/5', 'e/5', 'g/5'], duration: '2' } as IStaveNoteStruct)
        ...
        vf.StaveNote({ keys: ['f/4', 'g/4', 'a/4', 'b/4', 'c/5', 'e/5', 'g/5'], duration: '16' } as IStaveNoteStruct)
        ...
        vf.StaveNote({ keys: ['a/3', 'c/4', 'e/4', 'b/4', 'd/5', 'g/5'], duration: '1' } as IStaveNoteStruct)
          .addAccidental(0, newAccid('#'))
      ...
      notes.forEach(function(note, index) {
        Test.plotNoteWidth(vf.getContext(), note, 140);
      ...
      Test.plotLegendForNoteWidth(vf.getContext(), 480, 140);

      ok(true, 'Full Accidental');
    },

    cautionary: function(options) {
      const vf = Test.makeFactory(options, 800, 240);
      const stave = vf.Stave(<IFactoryParams>{ x: 0, y: 10, width: 780 });
      const score = vf.EasyScore();

      const accids = Object
        .keys(Vex.Flow.ACCIDENTALS)
        .filter(function(accid) {
          return accid !== '{' && accid !== '}';
        });
      ...
      const notes = accids
        .map(function(accid) {
          return vf
            .StaveNote(<IStaveNoteStruct>{ keys: ['a/4'], duration: '4', stem_direction: Stem.UP })
            .addAccidental(0, vf.Accidental({ type: accid } as IFactoryParams));
        });
    ...
    specialCases: function(options) {
      const vf = Test.makeFactory(options, 700, 240);
      const newAccid = makeNewAccid(vf);
      vf.Stave(<IFactoryParams>{ x: 10, y: 10, width: 550 });

      const notes = [
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['f/4', 'd/5'], duration: '1' })
          .addAccidental(0, newAccid('#'))
          .addAccidental(1, newAccid('b')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'g/4'], duration: '2' })
          .addAccidental(0, newAccid('##'))
          .addAccidental(1, newAccid('##')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['b/3', 'd/4', 'f/4'], duration: '16' })
          .addAccidental(0, newAccid('#'))
          .addAccidental(1, newAccid('#'))
          .addAccidental(2, newAccid('##')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['g/4', 'a/4', 'c/5', 'e/5'], duration: '16' })
          .addAccidental(0, newAccid('b'))
          .addAccidental(1, newAccid('b'))
          .addAccidental(3, newAccid('n')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['e/4', 'g/4', 'b/4', 'c/5'], duration: '4' })
          .addAccidental(0, newAccid('b').setAsCautionary())
          .addAccidental(1, newAccid('b').setAsCautionary())
          .addAccidental(2, newAccid('bb'))
          .addAccidental(3, newAccid('b')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['b/3', 'e/4', 'a/4', 'd/5', 'g/5'], duration: '8' })
    ...
    basicStemDown: function(options) {
      ...
      const notes = [
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'a/4'], duration: 'w', stem_direction: -1 })
          .addAccidental(0, newAccid('b'))
          .addAccidental(1, newAccid('#')),
    ...
    showNotes: function(note1, note2, stave, ctx, x) {
      ...
      Test.plotNoteWidth(ctx, note2, 15);
    },

    multiVoice: function(options) {
      const vf = Test.makeFactory(options, 460, 250);
      const newAccid = makeNewAccid(vf);
      const stave = vf.Stave(<IFactoryParams>{ x: 10, y: 45, width: 420 });
      const ctx = vf.getContext();

      stave.draw();

      let note1 = vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'a/4'], duration: '2', stem_direction: -1 })
        .addAccidental(0, newAccid('b'))
        .addAccidental(1, newAccid('n'))
        .addAccidental(2, newAccid('#'))
        .setStave(stave);

      let note2 = vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/5', 'a/5', 'b/5'], duration: '2', stem_direction: 1 })
        .addAccidental(0, newAccid('b'))
      ...
      note1 = vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'c/5'], duration: '2', stem_direction: -1 })
        .addAccidental(0, newAccid('b'))
        .addAccidental(1, newAccid('n'))
        .addAccidental(2, newAccid('#'))
        .setStave(stave);

      note2 = vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/5', 'a/5', 'b/5'], duration: '4', stem_direction: 1 })
        .addAccidental(0, newAccid('b'))
        .setStave(stave);

      this.showNotes(note1, note2, stave, ctx, 150);

      note1 = vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/4', 'c/5', 'd/5'], duration: '2', stem_direction: -1 })
        .addAccidental(0, newAccid('b'))
        .addAccidental(1, newAccid('n'))
        .addAccidental(2, newAccid('#'))
        .setStave(stave);
      ...
      Test.plotLegendForNoteWidth(ctx, 350, 150);
    ...
    microtonal: function(options) {
      ...
      const newAccid = makeNewAccid(vf);
      const ctx = vf.getContext();
      vf.Stave(<IFactoryParams>{ x: 10, y: 10, width: 650 });

      const notes = [
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'a/4'], duration: '1' })
          .addAccidental(0, newAccid('db'))
          .addAccidental(1, newAccid('d')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/4', 'e/4', 'f/4', 'a/4', 'c/5', 'e/5', 'g/5'], duration: '2' })
        ...
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['f/4', 'g/4', 'a/4', 'b/4', 'c/5', 'e/5', 'g/5'], duration: '16' })
          .addAccidental(0, newAccid('++'))
        ...
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['a/3', 'c/4', 'e/4', 'b/4', 'd/5', 'g/5'], duration: '1' })
          .addAccidental(0, newAccid('#'))
        ...
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['f/4', 'g/4', 'a/4', 'b/4', 'd/5', 'g/5'], duration: '16' })
          .addAccidental(0, newAccid('++-'))
          .addAccidental(1, newAccid('+-'))
      ...
      Test.plotLegendForNoteWidth(ctx, 580, 140);
    ...
    microtonal_iranian: function(options) {
      ...
      const notes = [
        ...
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['f/4', 'g/4', 'a/4', 'b/4'], duration: '16' })
          .addAccidental(0, newAccid('k'))
          .addAccidental(1, newAccid('k'))
    ...
    sagittal: function(options) {
      ...
      const notes = [
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/4', 'f/4', 'b/4', 'b/4'], duration: '4' })
        ...
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/4', 'f/4', 'a/4', 'b/4'], duration: '4' })
          .addAccidental(2, newAccid('accSagittal35LargeDiesisDown')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'g/4', 'c/5'], duration: '8' })
          .addAccidental(1, newAccid('accSagittal5CommaDown')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'g/4', 'b/4'], duration: '8' })
          .addAccidental(1, newAccid('b'))
      ...
      vf.Beam(<IFactoryParams>{ notes: notes.slice(2, 4) as Note[] });
      vf.Beam(<IFactoryParams>{ notes: notes.slice(6, 8) as Note[] });
    ...
    automaticAccidentals0: function(options) {
      ...
      notes[0].addModifier(0, vf.GraceNoteGroup(<IFactoryParams>{ notes: gracenotes }).beamNotes());
    ...
    automaticAccidentals1: function(options) {
      const vf = Test.makeFactory(options, 700, 150);
    ...
    autoAccidentalWorking: function() {
      function makeNote(noteStruct) { return new StaveNote(noteStruct); }
    ...
    factoryAPI: function(options) {
      ...
      function newAcc(type) { return vf.Accidental(<IFactoryParams>{ type: type }); }

      const notes = [
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['c/4', 'e/4', 'a/4'], duration: 'w' })
          .addAccidental(0, newAcc('b'))
          .addAccidental(1, newAcc('#')),

        vf.StaveNote(<IStaveNoteStruct>{ keys: ['d/4', 'e/4', 'f/4', 'a/4', 'c/5', 'e/5', 'g/5'], duration: 'h' })
        ...
        vf.StaveNote(<IStaveNoteStruct>{ keys: ['a/3', 'c/4', 'e/4', 'b/4', 'd/5', 'g/5'], duration: 'w' })
          .addAccidental(0, newAcc('#'))
